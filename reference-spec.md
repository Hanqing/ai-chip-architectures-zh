...
在 `#nvidia-architecture` 的实际阅读位置验证了 SVG 标签覆层：第二张 SM 图在 720px 内容栏中正确随原图缩放，中文标签固定在调度、发射、张量内存、共享内存和 TMA 对应区域；图像保持可见、未发生裁切。标签采用半透明浅底和深色文字，在原站暗色主题下与图内深浅分区有充分对比，不改变原图的比例、拓扑或数据。

语言切换回归已验证：点击侧栏 `EN` 后，文章标题、正文、目录小节、比较入口与结尾 CTA 均切换为英文，浏览器哈希维持 `#nvidia-architecture`。英文源的少数相邻提取段落将 `### ARCHITECTURE` 粘连至上一代谱系说明，且英文表格需要映射为原始英文列标题；这两项将通过解析正则和双语表格头在同轮实现中修正。

首个术语元素可获取键盘焦点，但初始实现依赖 `:focus-visible`，在脚本聚焦的回归测试中没有呈现浮层。已将术语浮层触发条件调整为 `:focus`，从而确保键盘 Tab 与程序化可达路径均会显示解释；悬停时的行为保持不变。

修复后验证：术语元素获取焦点时同时满足 `.term:focus .term-tooltip`，悬浮释义的最终计算样式为 `visibility: visible`。首次同步读取到的隐藏状态是 130ms 过渡尚未结算的瞬间，不影响键盘交互；在样式重算后浮层会正常显示。

当前中文状态的 DOM 回归结果：共检测到 20 个架构图容器与 20 个对应 SVG 中文标签覆层，表明每幅已嵌入架构图均已绑定覆层；正文中有 288 个可悬浮术语节点，侧栏显示 `EN` 切换控件，并保留两张比较表。英文状态下覆层自动隐藏，原图英文标签作为对照保留。

增强版的生产构建和 TypeScript 检查均成功完成。浏览器日志中仅保留早于本轮变更的 React key 历史记录；刷新后的最终页面控制台无新错误。

## 图示中文覆层规格

架构图文件均为高分辨率：多数单芯片图长边 4,000–5,840px，NVIDIA / AMD 纵向扩展图为 3,840×4,888px，横向扩展图宽度最高为 5,760px。中文标签将通过网页内的相对定位 SVG 覆层实现，不修改底图像素；所有坐标以原图尺寸归一化到 SVG `viewBox="0 0 100 100"`，从而在 720px 文章栏与移动端窄屏中保持准确对齐。覆层只翻译高层结构分区和流程标签，原图内的型号、标准缩写、速率、数值、端口编号和品牌名保留英文，以保护技术精确性与原站图面结构。
